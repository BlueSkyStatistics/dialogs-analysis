...
var localization = {
    en: {
        ...
        help: {
            ...
            This dataset is an updated version of the melanoma dataset in the MASS package.<br/><br/>
            <b>Competing Risks, one group</b>
            <br/>
            <br/>
            ...
            <br/>
            <br/>
            <b>Time:</b> For each subject, the length of time to the first event, if an event occurred, or last follow-up for events, if no events occurred.
            <br/><br/>
            ...
            <br/><br/>
            <b>Plot for a Single Event:</b> Whether you want to plot the cumulative incidence for a specific event by itself.  The Event Code corresponds to the event that will be plotted.
            <br/><br/>
...
class competingRisksOneGroup extends baseModal {
    constructor() {
        var config = {
            ...
            scale_color +
            scale_fill +
            theme(legend.position = "{{selected.legendpos | safe}}") +
            ...
            if ({{selected.cichkbox | safe}} & ci_type=="step") {
            ...
            if ({{selected.natriskchkbox | safe}}) {
            all_fit <- survfit(Surv({{selected.timevar | safe}},allevents) ~ 1, data=dataforfup)
            numatrisk <- ggsurvplot(fit=all_fit, risk.table=TRUE, xlim=plot_time_limits, break.time.by=plot_time_inc, risk.table.fontsize=4.2, tables.theme=theme_survminer(font.main=15), legend="none", legend.labs=" ", legend.title=" ", xlab="{{selected.timeaxislabel | safe}}")
            myplot <- cowplot::plot_grid(myplot, numatrisk$table + {{selected.themedropdown | safe}}, nrow=2, rel_heights=c(1-{{selected.risktableprop | safe}},{{selected.risktableprop | safe}}), align="v", axis="lr")
            }     
            ...
            if ({{selected.singleeventchkbox | safe}}) {
                ...
                # addition of number at risk
                if ({{selected.natriskchkbox | safe}}) {
                singleeventplot <- cowplot::plot_grid(singleeventplot, numatrisk$table + {{selected.themedropdown | safe}}, nrow=2, rel_heights=c(1-{{selected.risktableprop | safe}},{{selected.risktableprop | safe}}), align="v", axis="lr")
                }
                
                print(singleeventplot)  
            }          
`
        }
        var objects = {
